...
public class TaskData {
    ...
    public static Set<Task> getTasks(String owner){
        ...
        String user = ("ann,bob,carol".contains(owner.toLowerCase(Locale.ENGLISH))) ? owner : null;

        String selectedList = switch (owner.toLowerCase(Locale.ENGLISH)){
            case "ann" -> annsTasks;
            case "bob" -> bobsTasks;
        ...
        for (String taskData : selectedList.split("\n")){
            ...
            Status status = (data.length <= 3) ? Status.IN_QUEUE :
                    Status.valueOf(data[3].toUpperCase(Locale.ENGLISH).replace(' ', '_'));

            Priority priority = Priority.valueOf(data[2].toUpperCase(Locale.ENGLISH));
