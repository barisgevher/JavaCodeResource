...
//        week2Purchases.forEach((key, value) -> System.out.println(key + ": " + value));
        ...
        var previouslyEntry = datedPurchases.lastEntry();

        while (previouslyEntry != null){
            List<Purchase> lastDaysData = previouslyEntry.getValue();
            System.out.println(lastDate + " purchases : " + lastDaysData.size());

            LocalDate prevDate = datedPurchases.lowerKey(lastDate);
            previouslyEntry = datedPurchases.lowerEntry(lastDate);
            lastDate = prevDate;
        }
